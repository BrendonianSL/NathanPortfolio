        ...
        <section className='flex flex-col items-center pb-(--spacing-xl) pl-(--spacing-s) pr-(--spacing-s) md:px-(--spacing-m) lg:px(--spacing-m) gap-(--spacing-xxl)'>
            <article className="relative h-[calc(100vh-89px)] flex flex-col justify-center items-center">
                <div className="flex flex-col items-center justify-center">
                    <h1 className="text-center">NATHAN GORDON</h1>
                </div>
                <p className="absolute bottom-[60px] text-center w-full max-w-[400px]">
                    PENNSYLVANIA BASED DATA ANALYST PASSIONATE ABOUT TRANSFORMING COMPLEX DATA INTO CLEAR, ACTIONABLE INSIGHTS.
                </p>
            </article>
            <article>
            </article>
            <article className='w-full flex flex-col max-w-[82.5rem] gap-(--spacing-m)'>
                <div className='flex flex-col gap-(--spacing-s)'>
                    <span className='sectionLabel'>PROJECT SHOWCASE</span>
                    <h2>Projects Developed With Passion</h2>
                </div>
                    ...
                    <div className='flex flex-col gap-(--spacing-s) cursor-pointer w-full group'>
                        ...
                        <div className='flex flex-col gap-(--spacing-s)'>
                            <div className='flex flex-col gap-(--spacing-xs)'>
                                ...
                                <span className='text-alt-black2 text-[.875rem]'>JavaScript | HTML | CSS</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <article className='flex flex-col w-full max-w-[1320px] gap-(--spacing-l) lg:flex-row'>
                <div className='flex flex-col w-full gap-(--spacing-s)'>
                    ...
                    <h2>From Strategy To Execution, My Workflow Works!</h2>
                ...
                <div className='w-full grid gap-y-(--spacing-m) gap-x-(--spacing-m)'>
                    {workflow.map((element, index) => {
                        return (
                            <div key={index} className='flex flex-col gap-(--spacing-s) border-b-2 border-dashed border-shade pb-(--spacing-m)'>
            ...
            <article className='flex flex-col w-full max-w-[1320px] gap-(--spacing-m) lg:gap-(--spacing-m)'>
                <div className='flex flex-col w-full gap-(--spacing-s) border-b-2 border-shade border-dashed pb-(--spacing-m)'>
                    <span className='sectionLabel'>SKILLSET</span>
                    <h2>I transform data into <span className='text-beige'>actionable insights</span>, enabling businesses to make informed decisions and drive <span className='text-beige'>meaningful growth</span>.</h2>
                </div>
                <div className='flex flex-col lg:flex-row gap-(--spacing-s)'>
                    {services.map((element, index) => {
                        return (
                        <div key={index} className='flex flex-col gap-(--spacing-s) border-2 border-shade bg-[#1A1A1A] rounded-lg pt-(--spacing-m) pb-(--spacing-m) pl-(--spacing-m) pr-(--spacing-m) w-full'>
            ...
            <article className='w-full max-w-[1320px] flex flex-col gap-(--spacing-m)'> 
                <div className='w-full max-w-[1320px] border-b-2 pb-(--spacing-m) border-dashed border-shade flex flex-col gap-(--spacing-s)'>
                    <span className='sectionLabel'>About Me</span>
                    <h2 className=''>The Man Behind The Screen</h2>
                </div>
                    ...
                    <div className='w-full flex flex-col gap-(--spacing-m) justify-start items-start'>
                        ...
                        <div className='flex flex-wrap w-full gap-(--spacing-xs)'>
                            {techStack.map((element, index) => {
                                return (
                                    <div key={index} className='bg-beige text-alt-black rounded-lg px-4 py-2 text-[14px]'>
                                        {element.name}
                                    </div>
                                )
                            })}
